...
const contracts = [
  ...
  ['Links cards expose ordering and per-card style without first-card hardcoding', onlineStudioApp.includes("['style', '卡片樣式', 'select'") && onlineStudioApp.includes('dataset.moveCollection') && answersModule.includes("LINK_STYLES = ['primary', 'normal', 'subtle']") && liveRenderer.includes('item.style') && !liveRenderer.includes("index === 0 ? 'primary'")],
