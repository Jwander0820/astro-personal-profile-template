...
const [
  ...
  readFile(new URL('../src/components/ProfileHeader.astro', import.meta.url), 'utf8'),
  ...
  readFile(new URL('./profile-contract.mjs', import.meta.url), 'utf8'),
  readFile(new URL('./profile-content.mjs', import.meta.url), 'utf8'),
...
const contracts = [
  ...
  ['deprecated cover label remains input-compatible without being written or rendered', contentConfig.includes('archiveLabel: contentText.optional()') && !profileHeader.includes('archiveLabel') && !profileContentModule.includes('input.archiveLabel') && !liveRenderer.includes('archiveLabel') && !css.includes('profile-archive-label')],
  ...
  ['Content Collections coerce only finite numeric display text while keeping other types strict', contentConfig.includes("import { contentText, contentTextArray, contentTextMax } from '../scripts/content-text-schema.mjs'") && contentConfig.includes('displayName: contentText') && contentConfig.includes('title: contentText') && contentConfig.includes('imageAlt: contentTextMax(300)')],
  ...
  ['web embeds are editable collections under other features and use the shared block structure', onlineStudioPage.indexOf('panel-features') < onlineStudioPage.indexOf('id="embed-block-list"') && onlineStudioApp.includes('embedBlocks: {') && onlineStudioApp.includes('normalizeEmbedSource') && liveRenderer.includes('renderEmbedBlock') && profileRenderer.includes("block.data.layout === 'embed'")],
  ...
  ['local mode previews one transactional project plan before applying answers and images', onlineStudioPage.includes('id="save-project"') && onlineStudioApp.includes('requestProjectPlan') && onlineStudioApp.includes('applyProjectPlan') && onlineStudioApp.includes('expectedPlanToken: plan.token') && onlineStudioStyle.includes('[hidden] { display: none !important; }')],
  ['Studio exposes both schema-backed font scale controls and the formal preview applies them', onlineStudioPage.includes('data-bind="appearance.fontScale"') && onlineStudioPage.includes('data-bind="appearance.smallTextScale"') && previewBridge.includes('answers.appearance.fontScale') && previewBridge.includes('answers.appearance.smallTextScale')],
