...
const [
  ...
  readFile(new URL('../src/scripts/profile-preview-loader.js', import.meta.url), 'utf8'),
  readFile(new URL('../src/scripts/studio-draft.js', import.meta.url), 'utf8'),
...
const contracts = [
  ...
  ['formal and preview typography share font families and stylesheet selection', baseLayout.includes('getProfileFonts(bodyFont, displayFont)') && previewBridge.includes('getProfileFonts(appearance.bodyFont, appearance.displayFont)') && baseLayout.includes('id="profile-fonts"') && previewBridge.includes('stylesheet.getAttribute')],
  ['live Markdown uses a parser instead of inline regex substitution', liveRenderer.includes("import { markdownFragment } from './preview-markdown.js'") && !liveRenderer.includes('appendTextWithInlineMarkdown')],
  ['preview runtime is loaded only inside the marked Studio iframe', baseLayout.includes('profile-preview-loader.js') && previewLoader.includes('window.parent !== window') && previewLoader.includes('isStudioPreviewSearch(window.location.search)') && previewLoader.includes("import('./profile-preview-bridge.js')")],
  ['profile and fortune editors share scoped drafts and conflict detection', onlineStudioApp.includes('createDraftStore(bootstrap.draftScope') && fortuneStudioApp.includes('createDraftStore(bootstrap.draftScope') && studioDraft.includes('navigator.locks.request') && onlineStudioPage.includes('StudioDraftConflict') && fortuneStudioPage.includes('StudioDraftConflict')],
  ['Studio offers undo and referenced-only media exports', onlineStudioPage.includes('id="undo-draft"') && onlineStudioPage.includes('id="redo-draft"') && onlineStudioApp.includes('createDraftHistory') && onlineStudioApp.includes('referencedStudioImages(exported.result.answers, imageFiles)') && onlineStudioApp.includes('serializeStudioImages(referencedStudioImages(answers, imageFiles))')],
  ['ZIP imports validate before committing media and draft state', onlineStudioApp.includes('const nextState = parseImportedDraft') && onlineStudioApp.indexOf('bytes.length > 5 * 1024 * 1024') < onlineStudioApp.indexOf('await storeStudioMedia(media, bootstrap.draftScope)') && onlineStudioApp.indexOf('await storeStudioMedia(media, bootstrap.draftScope)') < onlineStudioApp.indexOf('state = nextState')],
  ...
  ['Links custom icons match formal and preview rendering with contained sizing', linkCard.includes('link.data.image') && linkCard.includes('withBase(link.data.image)') && linkCard.includes('size={24}') && liveRenderer.includes('const imageUrl = item.image ? assetHref(item.image)') && liveRenderer.includes('renderLinkCard(item, icons, assetHref)') && liveRenderer.includes('image.width = 24') && liveRenderer.includes('image.height = 24') && css.includes('.link-icon img { width: 24px; height: 24px; }') && css.includes('.socials img, .link-icon img { display: block; object-fit: contain; }')],
