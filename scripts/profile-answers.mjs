...
function atAnswerPath(path, validate) {
  try { return validate(); }
...
function assertUnique(items, key, label, path) {
  const values = items.map((item) => item[key]);
  const duplicate = values.findIndex((value, index) => values.indexOf(value) !== index);
  if (duplicate >= 0) {
    const error = new Error(`${label}不可重複。`);
    if (path) error.path = `${path}.${duplicate}.${key}`;
    throw error;
  }
  return items;
}
...
export function validateProfileAnswers(input) {
  ...
  const tagline = atAnswerPath('identity.tagline', () => assertStringArray(input.identity.tagline ?? [], '關鍵字', { max: 6 }));
  if (new Set(tagline).size !== tagline.length) throw new Error('關鍵字不可重複。');
  const identity = {
    displayName: atAnswerPath('identity.displayName', () => assertDisplayText(input.identity.displayName, '顯示名稱', { required: true, max: 80 })),
    title: atAnswerPath('identity.title', () => assertDisplayText(input.identity.title, '一句話身分', { max: 120 })),
    location: atAnswerPath('identity.location', () => assertDisplayText(input.identity.location, '地點', { max: 100 })),
    tagline,
    bio: atAnswerPath('identity.bio', () => assertProvidedDisplayText(input.identity.bio, '自我介紹', { max: 5000 })),
  };
  ...
  const socials = socialInput.map((item, index) => {
    ...
    const service = atAnswerPath(`socials.${index}.service`, () => assertSlug(item.service, '社群服務'));
    return {
      service,
      title: atAnswerPath(`socials.${index}.title`, () => assertProvidedDisplayText(item.title, '社群名稱', { max: 80 }) || SERVICE_DEFAULTS[service]?.title || service),
      url: atAnswerPath(`socials.${index}.url`, () => assertUrl(item.url, '社群網址')),
      icon: atAnswerPath(`socials.${index}.icon`, () => item.icon === undefined ? SERVICE_DEFAULTS[service]?.icon || 'arrow' : assertSlug(item.icon, '圖示名稱')),
    };
  });
  ...
  const sections = sectionInput.map((item, index) => {
    ...
    const image = item.image === undefined ? '' : atAnswerPath(`sections.${index}.image`, () => assertImageSource(item.image, '圖片來源', { required: true }));
    return {
      id: atAnswerPath(`sections.${index}.id`, () => assertSlug(item.id, '自介區塊 ID')),
      title: atAnswerPath(`sections.${index}.title`, () => assertDisplayText(item.title, '自介區塊名稱', { required: true, max: 80 })),
      description: atAnswerPath(`sections.${index}.description`, () => assertDisplayText(item.description, '自介區塊內容', { required: true, max: 2000 })),
      tags: atAnswerPath(`sections.${index}.tags`, () => assertStringArray(item.tags ?? [], '自介區塊標籤', { max: 8 })),
      ...(image ? { image } : {}),
    };
  });
  ...
  const imageBlocks = imageBlockInput.map((item, index) => {
    ...
    return {
      id: atAnswerPath(`imageBlocks.${index}.id`, () => assertSlug(item.id, '圖片板塊 ID')),
      title: atAnswerPath(`imageBlocks.${index}.title`, () => assertDisplayText(item.title, '圖片板塊標題', { required: true, max: 80 })),
      image: atAnswerPath(`imageBlocks.${index}.image`, () => assertImageSource(item.image, '圖片板塊圖片', { required: true })),
      imageAlt: atAnswerPath(`imageBlocks.${index}.imageAlt`, () => assertProvidedDisplayText(item.imageAlt, '圖片替代文字', { max: 300 })),
      description: atAnswerPath(`imageBlocks.${index}.description`, () => assertProvidedDisplayText(item.description, '圖片板塊文字', { max: 5000 })),
      placement: atAnswerPath(`imageBlocks.${index}.placement`, () => assertOptionalEnum(item.placement, IMAGE_BLOCK_PLACEMENTS, '圖片板塊位置', 'after-sections')),
      imageLayout: atAnswerPath(`imageBlocks.${index}.imageLayout`, () => assertOptionalEnum(item.imageLayout, IMAGE_BLOCK_LAYOUTS, '圖片板塊版型', 'full')),
      imageAspect: atAnswerPath(`imageBlocks.${index}.imageAspect`, () => assertOptionalEnum(item.imageAspect, IMAGE_BLOCK_ASPECTS, '圖片板塊比例', 'landscape')),
      imagePosition: atAnswerPath(`imageBlocks.${index}.imagePosition`, () => assertOptionalEnum(item.imagePosition, IMAGE_BLOCK_POSITIONS, '圖片板塊焦點', 'center')),
      tags: atAnswerPath(`imageBlocks.${index}.tags`, () => assertStringArray(item.tags ?? [], '圖片板塊標籤', { max: 8 })),
    };
  });
  ...
  const embedBlocks = embedBlockInput.map((item, index) => {
    ...
    return {
      id: atAnswerPath(`embedBlocks.${index}.id`, () => assertSlug(item.id, '網頁內嵌板塊 ID')),
      title: atAnswerPath(`embedBlocks.${index}.title`, () => assertDisplayText(item.title, '網頁內嵌板塊標題', { required: true, max: 80 })),
  ...
  assertUnique(socials, 'service', '社群服務', 'socials');
  assertUnique(links, 'id', '精選連結 ID', 'links');
  assertUnique(sections, 'id', '自介區塊 ID', 'sections');
  assertUnique(imageBlocks, 'id', '圖片板塊 ID', 'imageBlocks');
  assertUnique(embedBlocks, 'id', '網頁內嵌板塊 ID', 'embedBlocks');
  ...
  const mainColor = atAnswerPath('appearance.mainColor', () => assertThemeColor(appearance.mainColor ?? APPEARANCE_DEFAULTS.mainColor, 'appearance.mainColor'));
  const fontScale = atAnswerPath('appearance.fontScale', () => assertOptionalNumber(appearance.fontScale, 'appearance.fontScale', APPEARANCE_RANGES.fontScale, APPEARANCE_DEFAULTS.fontScale));
  const smallTextScale = atAnswerPath('appearance.smallTextScale', () => assertOptionalNumber(appearance.smallTextScale, 'appearance.smallTextScale', APPEARANCE_RANGES.smallTextScale, APPEARANCE_DEFAULTS.smallTextScale));
  ...
  const playlist = input.playlist === null || input.playlist === undefined ? null : {
    youtubePlaylistId: atAnswerPath('playlist.youtubePlaylistId', () => extractYoutubePlaylistId(input.playlist.youtubePlaylistId)),
    title: atAnswerPath('playlist.title', () => assertProvidedDisplayText(input.playlist.title, '播放清單名稱', { max: 80 }) || 'PLAY！'),
    description: atAnswerPath('playlist.description', () => assertProvidedDisplayText(input.playlist.description, '播放清單說明', { max: 500 }) || '按下唱針，隨機抽一首歌。'),
  };
  ...
  if (isObject(input.fortune)) {
    ...
    fortune = {
      title: atAnswerPath('fortune.title', () => assertProvidedDisplayText(input.fortune.title, '今日手氣標題', { max: 80 }) || '今日手氣'),
      description: atAnswerPath('fortune.description', () => assertProvidedDisplayText(input.fortune.description, '今日手氣說明', { max: 500 }) || '搖一搖，抽走今天的一點好運。'),
      fortunes: atAnswerPath('fortune.fortunes', () => validateFortuneBucket(input.fortune.fortunes)),
    };
  }
