...
- 主 Studio 頂部固定工具列新增最多 50 次修改的撤銷／重做，涵蓋卡片、圖片、匯入與還原預設；記錄限目前分頁，手機版以圖示按鈕顯示。
- 驗證失敗會切換步驟、展開卡片並聚焦錯誤欄位，保留可供輔助工具讀取的錯誤說明。
- 主 Studio 與籤詩頁共用專案隔離的草稿儲存與分頁衝突提示，使用者可選擇載入最新草稿或明確覆蓋。

### 修正

- 圖片替換或移除後，ZIP 與本機儲存只送出目前內容引用的圖片，避免舊圖片造成寫入失敗。
- 同名圖片在瀏覽器保存時避免覆蓋不同內容；圖片交易失敗不會留下失敗的記憶體檔案。
- 本機儲存期間暫停編輯，避免較晚回應覆蓋新輸入；寫入後仍可匯出上傳圖片或撤銷草稿。

### 效能

- 正式首頁僅在 Studio iframe 預覽模式動態載入 Markdown 與預覽 renderer，減少一般訪客下載及解析的 JavaScript。
