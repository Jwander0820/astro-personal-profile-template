...
- `http://localhost:4321/studio/`：Profile Studio 主編輯器；頂端 route 會在同一分頁前往籤詩與 Icon 工具。
...
「完成設定」集中 ZIP、JSON、匯入、還原與本機儲存。完整 ZIP 會包含回答檔和瀏覽器中的上傳圖片；單獨 JSON 會保存文字、籤桶、`/images/` 路徑與 HTTPS 網址，但不包含圖片 Blob。
...
`ONLINE_STUDIO_MODE=auto` 是正式部署預設值，會比對 `ONLINE_STUDIO_ALLOWED_REPOSITORIES` 與 `ONLINE_STUDIO_ALLOWED_SITES`；未命中的 fork／網域不生成 `/studio/` 及其籤詩、Icon 子路由與入口。`public` 明確公開，`off` 明確關閉。`npm run studio` 屬於本機專案模式，不受正式部署開關限制。
