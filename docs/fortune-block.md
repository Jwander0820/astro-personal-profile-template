...
一般使用者可先執行 `npm run studio`，再到「籤桶管理」新增、搜尋、編輯、停用、排序或刪除籤詩。Studio 會在整桶儲存前驗證所有欄位、避免覆蓋外部修改，並把上一次內容保存到 gitignored 的 `.studio-backups/fortunes-latest.json`；需要時可按「復原上一次籤桶」。

若偏好直接維護檔案，也可以依下列格式編輯 JSON。
...
- Studio：先在「籤桶管理」修改草稿，確認後按「儲存籤桶」一次寫入。
