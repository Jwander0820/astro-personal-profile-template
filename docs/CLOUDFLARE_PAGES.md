...
本專案輸出純靜態網站，可直接由 Cloudflare Pages 連接 GitHub 或 GitLab repository 建置。本機 Profile Studio 的寫入服務不會進入 `dist/`；允許公開後的 `/studio/` 是純前端設定產生器，可即時預覽圖片、操作唱盤並下載 ZIP 設定包，但不具備 repository 寫入能力，也不需要在 Cloudflare 保存 AI API key。
...
Cloudflare 建議用 `auto` 搭配正式網址 allowlist：

```text
ONLINE_STUDIO_MODE=auto
ONLINE_STUDIO_ALLOWED_SITES=https://<你的-project>.pages.dev
```

只有 `SITE_URL`／實際建置網址符合 `ONLINE_STUDIO_ALLOWED_SITES` 時，才會產生 `/studio/`、首頁 Links 卡片與頁尾連結。多個網址以逗號分隔。也可使用 `ONLINE_STUDIO_ALLOWED_REPOSITORIES=owner/repository` 精確允許 repository；Cloudflare 若未提供 `GITHUB_REPOSITORY`，應優先使用網址 allowlist。

`ONLINE_STUDIO_MODE=public` 會允許任何部署；`off` 會從正式建置移除 Studio。無論線上設定為何，`npm run studio` 的本機開發模式仍可使用。這是靜態網站的建置規則，不是密碼保護。
