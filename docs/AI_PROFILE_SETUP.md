...
允許公開 Studio 的部署會在首頁 Links 與頁尾提供 `/studio/` 入口。線上 Studio 不需要安裝 Node.js 或 npm，手機也能使用；它會從目前網站內容建立草稿，並支援 AI JSON 匯入、正式頁面即時預覽、圖片上傳或公開 HTTPS 圖片網址、唱盤試播、今日手氣試抽，以及下載包含 JSON 與圖片的 `profile-settings.zip`。
