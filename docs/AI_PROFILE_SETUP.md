...
本機啟動時，預設的 `4321`／`4322` 若被占用或保留，會自動選擇可用連接埠。請開啟終端機列出的 `Profile Studio` 網址；兩個編輯頁面的 API 位址會一起同步。

字型切換會同步載入預覽所需的外部字型；Markdown 的清單、斜體、引用、表格與註腳也可直接預覽，標題錨點與正式頁一致。程式碼上色及 Astro 圖片最佳化則以正式建置為準。ZIP 匯入先完整驗證，再以單次交易保存圖片；失敗時保留原草稿。回答檔文字含換行時，專案工具會寫成有效的 YAML 字串。
...
| 精選連結 | 標題、網址、簡短說明、排序與卡片樣式（一般、主色強調或低調），例如作品集、專案或文章；選填 `image` 自訂 icon（`/images/` 路徑或公開 HTTPS 圖片網址），未設定時使用 `icon` 內建圖示 |
